...
================================================================================================================= ]]
function modifier_item_jashin_on_attack_landed(keys)
	if keys.target.GetInvulnCount == nil then
		if keys.caster:HasModifier("modifier_item_jashin_unholy_rage") then  --The caster has Satanic's active on them.
			keys.ability:ApplyDataDrivenModifier(keys.attacker, keys.attacker, "modifier_item_jashin_unholy_rage_lifesteal", {duration = 0.03})
		end
		...
		keys.ability:ApplyDataDrivenModifier(keys.attacker, keys.attacker, "modifier_item_jashin_lifesteal", {duration = 0.03})
	end
end