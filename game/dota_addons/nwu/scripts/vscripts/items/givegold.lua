...
function givegold(keys)
	...
	local modifier_name = keys.ability.modfier_name
		if target:IsNeutralUnitType() and (target:GetTeamNumber() ~= caster:GetTeamNumber()) and keys.ability.current_stacks > 0  and not keys.ability.longCD then 
			-- add gold to caster
...
function initgivegold( keys )
	...
	keys.ability.current_stacks = 2
	keys.ability.shortCD = false
	keys.ability.longCD = false
	keys.caster:SetModifierStackCount( modifier_name, ability, 2)
end
--[[ ============================================================================================================
	Author: Dave
	Date: October 15, 2015
	I wasn' sure how recursion works with Timers, therefor i made 2 dirty functions. TODO CLEANER CODE
	Does check if the support item has less than 3 stacks, if so starts a timer which will add a stack after 10 seconds cd.
================================================================================================================= ]]
function resetCDFirst(keys)
	local max_stacks = keys.ability:GetLevelSpecialValueFor("gain_gold_max_stacks", (keys.ability:GetLevel() - 1))
